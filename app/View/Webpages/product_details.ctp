  ...
  <div class="productInfoDiv">
    ...
    <div class="productMiddleDeatil">
	 ...
	 $product=ClassRegistry::init('Product')->find('first', array('conditions' => array('product_id' =>$product_name_par)));
	  $sub=ClassRegistry::init('Subcategory')->find('first', array('conditions' => array('subcategory_id' =>$product['Product']['subcategory_id'],'status'=>'Active')));
	    $category=ClassRegistry::init('Category')->find('first', array('conditions' => array('category_id' =>$sub['Subcategory']['category_id'],'status'=>'Active')));
        ...
        <tr>
          <td valign="top" width="510" style="border-right:1px dotted #666;"><table cellpadding="0" cellspacing="0" border="0" width="100%">
              <tr>
                ...
                echo $this->Html->link(
                $this->Html->image('product/big/'.$images['Productimage']['imagename'],array("alt" => "Image",'width'=>'400')),$link,
                array('escape' => false,'class'=>'jqzoom','rel'=>'gal1')
                );
                ?></div>
                    <br/>
          ...
          <table cellpadding="0" cellspacing="0" border="0" width="100%">
			  ...
			  ?>
              <tr>
                <td colspan="9"><span class="b-star"><span style="width:<?php if(!empty($rating)) {  echo $count*20; }else{ echo '0';}?>%" class="rstar"></span></span>&nbsp; (<?php  if(!empty($reviewcount)) { echo $reviewcount;  }?> Reviews )</td>
              </tr>
              <tr>
                <td colspan="9">&nbsp;</td>
              </tr>
                ...
                <td colspan="9"><table cellpadding="0" cellspacing="0" border="0" width="100%">
                    ...
                    <?php   $ids=explode(',',$product['Product']['metal_color']); ?>
					 ...
					 $i=0;
					   foreach($ids as $ids) { 
					   ?>
                       <option value="<?php echo $ids;?>" <?php if($i==0) { echo 'selected="selected"'; } ?>><?php echo $ids;?></option>
                      ...
                      <td><?php
					   ...
					   ?>
                        <input type="radio" name="data[Product][goldpurity]" id="<?php echo $purity['Productmetal']['value'];?>" class="radio_gold_purity" value="<?php echo $purity['Productmetal']['value'];?>" <?php if(isset($_REQUEST['purity'])){ echo ($_REQUEST['purity']==$purity['Productmetal']['value']?'checked="checked"':'');}else{echo ($i==0?'checked="checked"':'');};?> /><?php echo $purity['Productmetal']['value'];?>K 
                      <?php $i++; } ?>
              ...
              <tr>
				...
				$i=0;
					...
					if($category['Category']['category']!="Bangles"){
					 ...
					 ?>
                    <option value="<?php echo $st?>" <?php if($i==0) { echo 'selected="selected"'; } ?>><?php echo $val;?></option>
                   <?php $i++;} ?></select>
                   </td></tr>
                   <?php  } ?>
                <tr>      <td colspan="9">&nbsp;</td></tr>
              <tr>
                <td colspan="5" style="text-align:left;"><strong>Expected Date of Delivery and COD Availabiity Update :</strong></td>
               <td colspan="5"><form id="deliveryForm" name="myForm" method="post" style="margin-top: 20px; margin-left: 10px;"><input placeholder="Enter pincode" name="pincode" type="text" class="validate[required,minSize[6],custom[integer]] pincode" onkeypress="return intnumbers(this, event)" maxlength="6">&nbsp;&nbsp;&nbsp;<input type="submit" name="update" class="update" value="Update" id="update" /></form>&nbsp;&nbsp;&nbsp; </td>
                </tr>
                <tr><td colspan="9" style="text-align:center;"><span class="delivery_date"></span></td>
             ...
             <?php
				 ...
				 $productimages=ClassRegistry::init('Productimage')->find('first', array('conditions' => array('product_id' =>$productnew['Product']['product_id'],'status'=>'Active')));
					 ...
					 if(!empty($subcategory)){
						 $subcatname=str_replace(' ','_',$subcategory['Subcategory']['subcategory']);
					 }else{
						 $subcatname='all_items';
					 }
				 ?>
			  <div class="slide">
                <a href="<?php echo BASE_URL;?><?php echo str_replace(' ','_',$category['Category']['category'])."/".$subcatname."/".$productnew['Product']['product_id']."/".$Product_product_name;?>"><img src="<?php echo  $imagelink; ?>"/></a>
            ...
            <!--  <a href="<?php echo BASE_URL;?>webpages/product_details/<?php echo $productnew['Product']['product_id'];?>"><?php echo $this->Html->image($imagelink,array('border'=>0,'alt'=>'logo') ); ?></a>--></div>
              <?php }?>
                ...
                <table cellpadding="0" cellspacing="0" border="0" width="100%">
                	...
                	<tr><td colspan="3">&nbsp; </td></tr>
                	<tr>
                    	<td colspan="3"><h3>Have a Question?</h3></td>
    ...
    <div class="best_seller">
      ...
      <table cellpadding="0" cellspacing="0" border="0" width="50%" align="center">
		...
		?>
          <td align="center" width="300"><p style="height: 150px; overflow: hidden"><?php echo $this->Html->image('testimonial/'.$customer['Testimonial']['image'],array('border'=>0,'alt'=>'logo','height'=>'100%') ); ?></p>
...
foreach($browse_product as $browse_product){
		 ...
		 $Product_product_name=str_replace(" ","_",$browse_product['Product']['product_name']);
		  ...
		  ?>
           ...
           <a href="<?php echo BASE_URL;?><?php echo str_replace(' ','_',$category['Category']['category'])."/".$subcat."/".$browse_product['Product']['product_id'].'/'.$Product_product_name;?>">
